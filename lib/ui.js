...
export async function fetchAndRenderLists(container, newsItemLimit) {
  ...
  fetchAndRenderCategory(container, parent, fetchLink);
}

let app = document.querySelector('.layout__main');

document.body.addEventListener('click', function(event) {
  let tag = event.target;
  console.log("user clicks!!");
  if (tag.tagName == 'A' && tag.href && event.button == 0) {
    console.log("user clicks on a href!!");
    ...
    if (tag.origin == document.location.origin) {
      ...
      if (app.capableOfRendering(newPath)) {
        ...
        console.log("app is handling the path at: " + newPath);
        app.render(newPath);
...
export async function fetchAndRenderCategory(
  ...
  if(window.location.search !== ""){ // ef er ekki á forsíðu
